...
export const TreeItem = ({ node, selected, active, depth }: TreeItemProps) => {
  ...
  const expandIcon = node.type !== "repo" 
    ? (node.expanded ? "▾ " : "▸ ") 
    : ""
      ...
      <box flexDirection="column" flexGrow={1}>
        ...
        {repo.analyzedAt && (
          <text fg={colors.fgDim}>
            Analyzed: {formatRelative(repo.analyzedAt)}
          </text>
        )}
        {repo.commitSha && (
          <text fg={colors.fgDim}>Commit: {repo.commitSha.slice(0, 8)}</text>
        )}
...
export const Header = () => (
  ...
  >
    ...
    <box alignItems="flex-end">
      ...
      <text fg={colors.accent} marginLeft={2}>[?]</text>
    </box>
  </box>
)
```
