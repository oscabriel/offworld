  ...
  return [...dirGroups.entries()]
    ...
    .map(([dir, files]) => {
      ...
      const keyFiles = files
        .sort((a, b) => (b.exports.length + b.functions.length) - (a.exports.length + a.functions.length))
        .slice(0, 5)
...
function detectPrimaryLanguage(parsedFiles: Map<string, ParsedFile>): string {
  ...
  for (const parsed of parsedFiles.values()) {
    const lang = typeof parsed.language === 'string' 
      ? parsed.language 
      : parsed.language.toString();
...
export function detectChanges(
  ...
  const keyFileChanged = [...added, ...modified].some(p => 
    keyFiles.some(kf => p.endsWith(kf))
  );

  const changeRatio = (added.length + modified.length + deleted.length) / 
    (currentFiles.size || 1);
  ...
  return {
    ...
    stats: {
      ...
      symbolsExtracted: [...parsedFiles.values()].reduce(
        (sum, p) => sum + p.functions.length + p.classes.length, 0
      ),
