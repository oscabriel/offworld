...
describe("projectInitHandler map write behavior", () => {
	...
	it("does not update project map when all installs fail", async () => {
		...
		mocks.resolveDependencyRepo.mockResolvedValue({ dep: "foo", repo: "owner/repo-a", source: "npm" });
		mocks.matchDependenciesToReferencesWithRemoteCheck.mockResolvedValue([
			{ dep: "foo", repo: "owner/repo-a", status: "generate", source: "npm" },
		]);
		...
		expect(mocks.logWarn).toHaveBeenCalledWith("No references were installed. Project map was not updated.");
	});
	...
	it("writes project map only for successfully installed repos", async () => {
		...
		mocks.matchDependenciesToReferencesWithRemoteCheck.mockResolvedValue([
			{ dep: "pkg-a", repo: "owner/repo-a", status: "generate", source: "npm" },
			{ dep: "pkg-a-dup", repo: "owner/repo-a", status: "generate", source: "npm" },
			{ dep: "pkg-b", repo: "owner/repo-b", status: "generate", source: "npm" },
		]);
		...
		expect(mocks.pullHandler).toHaveBeenCalledTimes(2);
		expect(mocks.pullHandler).toHaveBeenNthCalledWith(
			1,
			expect.objectContaining({ repo: "owner/repo-a" }),
		);
		expect(mocks.pullHandler).toHaveBeenNthCalledWith(
			2,
			expect.objectContaining({ repo: "owner/repo-b" }),
		);
