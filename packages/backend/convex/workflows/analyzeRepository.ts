 ...
 *
 * Steps:
 * 1. Fetch GitHub metadata (owner, name, description, language, default branch)
 * 2. Check if repo exists (determines new index vs re-index)
 * 3. If re-indexing: clear RAG namespace, entities, issues, conversations; reset repo record
 *    If new: create repo record
 * 4. Fetch file tree from GitHub
 * 5. Ingest files into RAG (chunking + embeddings)
 * 6. Generate summary with Gemini (using RAG context)
 * 7. Generate architecture iteratively (2-5 iterations based on repo size):
 *    - Iteration 1: packages/directories
 *    - Iteration 2: modules/services
 *    - Iteration 3+: components/utilities
 * 8. Consolidate entities (filter 50+ → 5-15 major entities by importance)
 * 9. Synthesize architecture narrative + generate C4 diagrams (architecture, data flow, routing)
 * 10. Update DB with architecture, narrative, metadata, diagrams
 * 11. Analyze issues (fetch, analyze with AI, store batch)
 * 12. Analyze pull requests (fetch, analyze with AI, store batch)
 * 13. Finalize (mark as completed)
 */
